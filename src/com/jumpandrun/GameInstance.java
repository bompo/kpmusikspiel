		...
		if (enemies.size <= 5) {
			for (Block block : blocks) {
				if (block instanceof EnemySpawner) {
					if (block.position.y > player.position.y - 10 && block.position.y < player.position.y + 20) {
...
//						}
	...
	public void flagBullets() {
		// List<Contact> contactList = world.getContactList();
		/*
		 * Iterator<Body> it = world.getBodies(); while(it.hasNext()) { Body b =
		 * it.next(); if(b.getUserData() instanceof Bullet) {
		...
		for (int i = 0; i < contactList.size(); i++) {
			...
			if (contact.isTouching()) {
				...
				// instanceof Player)) {
				// ((Ammo)a).hit = true;
				// } else if(b instanceof Ammo && !(a instanceof Ammo) && !(a
				// instanceof Player)) {
				// ((Ammo)b).hit = true;
				// }
			}
	...
	public void update(float delta) {
		...
		if (player.jump) {
			...
			if (grounded) {
				...
				// player.body.setLinearVelocity(vel.x,
				// vel.y+1*(100000000-(System.nanoTime() -
				// lastGroundTime))/100000000f);
			}
		}
...
//						if (((Block) contact.getFixtureB().getBody().getUserData()).position.y > ((Enemy) contact.getFixtureA().getBody().getUserData()).position.y - 1.0f
//								&& ((Block) contact.getFixtureB().getBody().getUserData()).position.y < ((Enemy) contact.getFixtureA().getBody().getUserData()).position.y - 0.5f) {
//							if (((Block) contact.getFixtureB().getBody().getUserData()).position.x > ((Enemy) contact.getFixtureA().getBody().getUserData()).position.x - 1.0f
//									&& ((Block) contact.getFixtureB().getBody().getUserData()).position.x < ((Enemy) contact.getFixtureA().getBody().getUserData()).position.x + 1.0f) {
//								((Enemy) contact.getFixtureA().getBody().getUserData()).direction.x = -((Enemy) contact.getFixtureA().getBody().getUserData()).direction.x;
//							}
//						}
//					}
//				}
...
//						if (((Block) contact.getFixtureA().getBody().getUserData()).position.y > ((Enemy) contact.getFixtureB().getBody().getUserData()).position.y - 1.0f
//								&& ((Block) contact.getFixtureA().getBody().getUserData()).position.y < ((Enemy) contact.getFixtureB().getBody().getUserData()).position.y - 0.5f) {
//							if (((Block) contact.getFixtureA().getBody().getUserData()).position.x > ((Enemy) contact.getFixtureB().getBody().getUserData()).position.x - 1.0f
//									&& ((Block) contact.getFixtureA().getBody().getUserData()).position.x < ((Enemy) contact.getFixtureB().getBody().getUserData()).position.x + 1.0f) {
//								((Enemy) contact.getFixtureB().getBody().getUserData()).direction.x = -((Enemy) contact.getFixtureB().getBody().getUserData()).direction.x;
//							}
//						}
//					}
//				}
